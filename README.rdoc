...
If necessary change the database settings in you config file database.yml to match 
...
you can for instance browse, open, edit and save the config files by using nautilus: 
...
If this command fails with message ...
 ...
 rm -r sftp://ostraaten@192.168.1.42/home/ostraaten/.rvm/gems/ruby-1.9.2-p290@global
...
To not use email at all add the following entry to setup_mail.rb
...
Apache2 configuration for you environment that you want to take note of.
	
 passenger-install-apache2-module

Depending on the current version the command outputs something like 
...
Now you should be able to see EPF Wiki when you navigate to http://localhost:3000

NOTE: The passenger version number may be different, currently it is 2.2.7. You may have to change it in the paths above.

=== Tidy

Install HTML Tidy. This is used for making nice visual difference analysis 
